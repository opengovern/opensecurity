...
import (
	...
	runner2 "github.com/opengovern/opensecurity/jobs/compliance-runner-job"
	...
	model2 "github.com/opengovern/opensecurity/services/scheduler/db/model"
...
func (h HttpServer) Register(e *echo.Echo) {
	...
	v3.GET("/tables/:table/described/count", httpserver.AuthorizeHandler(h.GetResourceTypeDescribedCount, apiAuth.ViewerRole))
}
...
func (h HttpServer) GetComplianceJobsHistory(ctx echo.Context) error {
	...
	if request.SortBy != nil {
		...
		case "datetime", "updated_at", "updatedat":
			sort.Slice(jobsResults, func(i, j int) bool {
				return jobsResults[i].LastUpdatedAt.After(jobsResults[j].LastUpdatedAt)
			})
		case "created_at", "createdat":
			sort.Slice(jobsResults, func(i, j int) bool {
...
//	@Security	BearerToken
//	@Tags		scheduler
//	@Param		query_id	path	string	true	"Policy ID"
//	@Produce	json
