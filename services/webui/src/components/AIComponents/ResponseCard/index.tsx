...
const KResponseCard: FunctionComponent<any> = ({
  ...
  return (
    <>
      <div key={key} className="    flex-col gap-2   flex justify-start items-start max-w-[95%]">
                  ...
                  {error && error != '' ? (
                    <>
                      <div className="rounded-3xl    p-2 px-4 my-2 text-red-700 text-left sm:w-fit w-full flex flex-row gap-2">
                        {/* <RiAlertLine /> */}
                        {pre_loaded ? error : useAnimatedText(error, 3).text}
                      </div>
                    </>
                  ) : (
                    <>
                        ...
                        {clarify_needed ? (
                          <>
                            <span className="text-slate-800 dark:text-slate-200">
                              Please answer below questions to clarify query.{' '}
                            </span>

                            <span className="text-slate-800 dark:text-slate-200">
                              {pre_loaded ? clarify_questions.join(',') : useAnimatedText(clarify_questions.join(','), 2).text}
                            </span>
                          </>
                        ) : (
                          <>
                            <span className="text-slate-800 dark:text-slate-200">
                              {pre_loaded ? showText : useAnimatedText(showText, 0.5).text}
                            </span>
                          </>
                        )}
                        {!pre_loaded && animatedtText.done && (
                          <>
                            <KTable
                              ref={ref}
                              scroll={scroll}
